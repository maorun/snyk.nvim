                ...
                on_exit = function(job, exit_code)
                    vim.schedule(function()
                        if exit_code == 0 then
                            ...
                            utils.printWithoutHistory('Snyk npm install failed. Exit code: ' .. exit_code)
                            afterCheck(nil) -- Indicate Snyk is not available
                        end
                    end)
                end,
        ...
        on_exit = function(_, exit_code) -- Ignoring signal argument
            vim.schedule(function()
                ...
                if apiKey == '' or apiKey == 'null' or exit_code ~= 0 then -- Also check for 'null' string
                    utils.printWithoutHistory('Snyk not yet authenticated (API key is empty or error). Attempting auth...')
                    Job:new({
                        command = snykCommand,
                        args = { 'auth' },
                        ...
                        on_exit = function(_, auth_exit_code)
                            vim.schedule(function()
                                if auth_exit_code == 0 then
                                    utils.printWithoutHistory('Snyk auth process initiated. Please check your browser.')
                                else
                                    utils.printWithoutHistory('Snyk auth command failed. Exit code: ' .. auth_exit_code)
                                end
                            end)
                        end,
                    }):start()
